...
namespace intelli_tutor_frontend.TeacherSide
{
    internal class TeacherCourseContent
    {
        ...
        public async void TeacherMyCoursesContentShow(WeekModel weekData, FlowLayoutPanel flowLayoutPanel, Label formName)
        {
            ...
            addPictureBox.Dock = DockStyle.None; // Set Dock to None
            addPictureBox.Anchor = AnchorStyles.Right; // Anchor to the right
            addPictureBox.Margin = new Padding(10, 20, 10, 20);
            addPictureBox.Cursor = Cursors.Hand;
            addPictureBox.MouseEnter += AddPictureBox_MouseEnter;
            addPictureBox.MouseLeave += AddPictureBox_MouseLeave;
            addPictureBox.Click += addPictureBox_Click;


            mainPanel.Controls.Add(addPictureBox, 0, 2);
            ...
            else
            {
                ...
                foreach (var item in contentlist)
                {
                    ...
                    deleteButton.Top = 15;
                    deleteButton.Padding = new Padding(5, 5, 5, 5); // Adjust padding
                    deleteButton.Font = new Font("Segoe UI Semibold", 12F);
                    deleteButton.BackColor = Color.DarkSlateBlue;
                    deleteButton.ForeColor = Color.White;
                    deleteButton.Click += async (sender, e) =>
                    {
                        DialogResult confirmation = MessageBox.Show("Are you sure you want to delete this content?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                        if (confirmation == DialogResult.Yes)
                        {

                            if (item.content_type == "Problem")
                            {
                                int problemId = await  problemApi.getProblemId(item.content_id);
                                if(problemId != -1)
                                {
                                    await testCasesApi.deleteTestCaseByProblemId(problemId);
                                    await problemApi.deleteProblemById(problemId);
                                }
                                await contentApi.deleteContentById(item.content_id);

                                MessageBox.Show("Content Deleted Successfully.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                flowLayoutPanel.Controls.Clear();
                    ...
                    buttonPanel.Controls.Add(deleteButton, 1, 0);
        ...
        {
            // Show the message when the mouse enters the PictureBox
            ToolTip toolTip = new ToolTip();
            toolTip.SetToolTip(addPictureBox, "Add New Content");
        }

        // MouseLeave event handler
        private void AddPictureBox_MouseLeave(object sender, EventArgs e)
        {
            // Hide the message when the mouse leaves the PictureBox
            ToolTip toolTip = new ToolTip();
            toolTip.Hide(addPictureBox);
        }
