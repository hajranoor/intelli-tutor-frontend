...
namespace intelli_tutor_frontend.TeacherSide
{
    internal class SuperAdminCourseContent
    {
        ...
        MainTestCaseApi mainTestCasesApi = new MainTestCaseApi();

        FlowLayoutPanel newFlowLayoutPanel = new FlowLayoutPanel();
        Label newFormName = new Label();
        MainWeekModel newWeekData = new MainWeekModel();

        PictureBox addPictureBox = new PictureBox();

        public async void ShowSuperAdminCourseContent(MainWeekModel weekData, FlowLayoutPanel flowLayoutPanel, Label formName)
        {
            ...
            mainContentList = await mainContentApi.getMainContentByWeekId(weekData.week_id);
            ...
            else
            {
                ...
                int count = 0;
                foreach (var item in mainContentList)
                {
                    Panel outerPanel = new Panel();
                    ...
                    viewButton.Click += (sender, e) =>
                    {
                        ...
                        {
                            SuperAdminViewProblem superAdminViewProblem = new SuperAdminViewProblem();
                            superAdminViewProblem.ShowSuperAdminViewProblem(item, flowLayoutPanel, formName, weekData);
                        }
